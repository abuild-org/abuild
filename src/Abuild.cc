...
std::string const Abuild::ABUILD_VERSION = "1.0.b2-prerelease";
...
Abuild::help()
{
    ...
    h("with a - is treated as an option.");
    h("");
    ...
    h("  --build=set |     specify a build set; target is applied to each item");
    h("    -b set");
    h("  --with-deps | -d  short-hand for --build=current");
    h("  --clean=set |     specify a clean set; abuild clean is run for each item");
    h("    -c set");
    ...
    h("  --full-integrity  check abuild's guarantee for all items");
    ...
    h("  --make-jobs[=n]   passes the -j flag onto make allowing each make to use");
    h("                    up to n jobs; omit n to let it use as many as it can");
    h("  --no-abuild-logger  suppress use of AbuildLogger for ant");
    ...
    h("  deps              all dependencies of the current item");
    h("  current           the current item");
    h("");
    h("All build/clean sets are automatically expanded to include the");
    h("recursively expanded dependency list of the items defined above.");
    h("");
    ...
    h("");
    h("For --build=all only (not --clean=all or any other build set),");
    h("non-local items whose dependencies are shadowed locally are excluded.");
    h("See documentation for a full discussion.");
    h("");
    h("The following additional argument is recognized and is passed along");
    h("to gnu make:");
    h("");
    h("  -n |              print what make would do without doing it");
    h("    --just-print |");
    h("    --dry-run |");
    h("    --recon");
    h("");
